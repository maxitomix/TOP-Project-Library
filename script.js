...
function displayBooks() {
    ...
    myLibrary.forEach(book => {
      ...
      bookRow.innerHTML = `
        ...
        <td>${book.author}</td>
        <td>${book.pages}</td>
        <td><input type="checkbox" class="read-checkbox" ${book.isRead ? 'checked' : ''} data-index="${myLibrary.indexOf(book)}"></td>
  ...
  const checkbox = document.querySelectorAll('.read-checkbox');
  checkbox.forEach(box => {
    box.addEventListener('click', () => {
      const libraryIndex = box.dataset.index;
      myLibrary[libraryIndex].isRead = !myLibrary[libraryIndex].isRead;
      displayBooks();
    })
  })
}
